...
Не реализованы. Дальнейшие этапы (зональные Encounter, состояния рисунков последствий, LOD/фильтры AM-06 в полном объёме — теперь, когда GetActiveStage существует, приоритет подписей можно на него опереть, — память/бюджет текстур) по-прежнему требуют либо реальных арт-ассетов, либо длительной ручной Unity-проверки по §19, либо того и другого.
