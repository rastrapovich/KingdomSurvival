...
>
> Актуальный общий канон: `KINGDOM_SURVIVAL_GAME_CONCEPT_CANON_RU_v1_46.md`.
>
...
**Основной этап — ПР-12 «Свободная игра»** (канон v1.45 §6.0; механика последствий и направление прогрессии уточнены в v1.46 §25). Базовый цикл Дома, похода и возвращения работает, но содержательные поводы для следующего выхода и заметные отложенные последствия ещё требуют авторской работы. «Дом на чужой воде» сохранён как первая кампания кризиса; его разработка приостановлена. Подробности ниже о N01–N17 и P09–P13 — история разработки главы, а не текущий приоритет.

На удалённом `main` перед этой документационной правкой (`11c9585`) в действующем дорожном пуле **9 встреч**: 8 коротких сцен (4 с проверкой) и «Тёплая овца». Описание 20 малых встреч в §1С относится к состоянию до последующего списания 12 сцен. Следующая практическая задача — аудит этих действующих встреч по §25 канона, затем одна причинная цепь `поход → выбор → возвращение → изменившийся Дом/человек → новый выход` и маленький каталог особенностей с реальными применениями.
