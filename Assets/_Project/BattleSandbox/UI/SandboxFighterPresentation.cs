...
namespace KingdomSurvival.BattleSandbox
{
    internal static class SandboxFighterCardFactory
    {
        ...
        private static UnitDatabaseAsset portraitDatabase;
            ...
            Action onDetails)
        {
            ...
            {
                SandboxInitiativePortrait fallback = new SandboxInitiativePortrait(
                    unit.Role,
                    unit.Team,
                    damaged);
                fallback.style.position = Position.Absolute;
                fallback.style.left = 0f;
                fallback.style.right = 0f;
                fallback.style.top = 0f;
                fallback.style.bottom = 0f;
    ...
    internal sealed class SandboxFighterDetailsView
    {
        ...
        public SandboxFighterDetailsView(VisualElement root)
        {
            ...
            VisualElement portraitViewport = new VisualElement();
            portraitViewport.style.position = Position.Absolute;
            portraitViewport.style.left = 12f;
            portraitViewport.style.right = 12f;
            portraitViewport.style.top = 12f;
            portraitViewport.style.bottom = 72f;
            portraitViewport.style.overflow = Overflow.Hidden;
            ...
            portraitImage.style.left = 0f;
            portraitImage.style.right = 0f;
            portraitImage.style.top = 0f;
            portraitImage.style.bottom = 0f;
            portraitViewport.Add(portraitImage);
            ...
            attackValue = CreateStatRow(
                ...
                "АТАКА",
                "—",
                "Если Атака выше эффективной Защиты цели, каждый пункт преимущества добавляет 25% к базовому Урону. Итоговый множитель ограничен ×5.");
            defenseValue = CreateStatRow(
                info,
                "ЗАЩИТА",
                "—",
                "Если эффективная Защита выше Атаки врага, каждый пункт преимущества снижает его базовый Урон на 12,5%. Снижение ограничено 70%; стойка временно добавляет +2.");
            damageValue = CreateStatRow(
                info,
                "УРОН",
                "—",
                "Базовая сила одного удара или выстрела. После сравнения Атаки и эффективной Защиты она умножается на процентный коэффициент, округляется вниз и не может стать меньше 1.");
